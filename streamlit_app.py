...
# ─────────────────────────────────────────────
# Strategy alternatives (3 runs, different objectives)
# ─────────────────────────────────────────────

def _run_strategy_alternatives(
    df_active, n_funds, step, include, constraint, targets,
    locked_fund, locked_weight_pct,
    current_managers: list,
):
    """
    Run find_best_solutions 3 times with different objective weights.
    Returns dict: {conservative, operational, aggressive}
    """
    base_kw = dict(
        df=df_active, n_funds=n_funds, step=step,
        include=include, constraint=constraint, targets=targets,
        locked_fund=locked_fund, locked_weight_pct=locked_weight_pct,
    )

    # ── A. Conservative: max service + prefer current managers ───────────
    # Filter df to prefer current managers (if any known)
    df_conservative = df_active.copy()
    if current_managers:
        cur_set = {str(m).strip().lower() for m in current_managers if m}
        # Give a boost: add current-manager funds to the front so they appear first
        mask_cur = df_conservative["manager"].str.lower().str.strip().isin(cur_set)
        df_conservative = pd.concat([
            df_conservative[mask_cur],
            df_conservative[~mask_cur],
        ], ignore_index=True)

    sols_c, _ = find_best_solutions(
        **{**base_kw, "df": df_conservative,
           "primary_rank": "שירות ואיכות",
           "mix_policy": "מותר לערבב מנהלים"}
    )

    # ── B. Operational: prefer internal changes (same managers) ──────────
    mix_policy_op = "אותו מנהל בלבד" if current_managers else "מותר לערבב מנהלים"
    sols_o, _ = find_best_solutions(
        **{**base_kw,
           "primary_rank": "שירות ואיכות",
           "mix_policy": mix_policy_op}
    )
    # Fallback if same-manager constraint yields nothing
    if (sols_o is None or sols_o.empty) and mix_policy_op != "מותר לערבב מנהלים":
        sols_o, _ = find_best_solutions(
            **{**base_kw, "primary_rank": "שירות ואיכות",
               "mix_policy": "מותר לערבב מנהלים"}
        )

    # ── C. Aggressive: pure target accuracy, ignore service ──────────────
    # Set all constraints to soft so optimizer chases the target hard
    soft_constraint = {k: ("רך", "בדיוק") for k in constraint}
    sols_a, _ = find_best_solutions(
        **{**base_kw,
    ...
    # ── D. Cost-efficient: best weighted score with cost awareness ────────
    # Uses same run as conservative but picks min-service-cost fund
    # (we just pick the max-sharpe result from the aggressive run)
    sols_sh, _ = find_best_solutions(
        **{**base_kw,
           "primary_rank": "שארפ",
           "mix_policy": "מותר לערבב מנהלים"}
    )
    ...
    def _best(sols, rank):
        if sols is None or sols.empty:
            return None
        if rank == "שירות ואיכות" and "שירות משוקלל" in sols.columns:
            return sols.loc[sols["שירות משוקלל"].idxmax()].to_dict()
        if rank == "שארפ" and "שארפ משוקלל" in sols.columns:
            return sols.loc[sols["שארפ משוקלל"].idxmax()].to_dict()
        return sols.loc[sols["score"].idxmin()].to_dict()

    return {
        "conservative": _best(sols_c, "שירות ואיכות"),
        "operational":  _best(sols_o, "שירות ואיכות"),
        "aggressive":   _best(sols_a, "דיוק"),
        "sharpe":       _best(sols_sh, "שארפ"),
    }
...
def render_product_selector():
    ...
    # ── Row layout: optimizer buttons LEFT, client button RIGHT ──────────
    nav_cols = st.columns([1.2, 1.4, 1.2, 1.2, 1.4, 0.15, 1.8])

    _optimizer_btns = [
        ("c1", "🏦 קרנות השתלמות",  "קרנות השתלמות",  "pt_funds"),
        ("c2", "📋 פוליסות חיסכון", "פוליסות חיסכון",  "pt_policies"),
        ("c3", "🏛️ קרנות פנסיה",   "קרנות פנסיה",    "pt_pension"),
        ("c4", "💼 קופות גמל",      "קופות גמל",       "pt_gemel"),
        ("c5", "📈 גמל להשקעה",    "גמל להשקעה",     "pt_gemel_inv"),
    ]
    for (_, label, val, key), col in zip(_optimizer_btns, nav_cols[:5]):
        with col:
    ...
    # ── Prominent client portfolio button on the RIGHT ────────────────────
    with nav_cols[6]:
        is_client = (pt == "תיק לקוח")
        btn_style = (
        ...
        st.markdown(
            f"<div style='{btn_style}text-align:center'>📊 ניתוח תיק לקוח</div>",
            unsafe_allow_html=True,
        )
        # Invisible Streamlit button for click detection
        if st.button("📊 ניתוח תיק לקוח", type="primary" if is_client else "secondary",
                     use_container_width=True, key="pt_client",
                     help="ניתוח מקיף של תיק ההשקעות הנוכחי של הלקוח"):
            if not is_client:
                st.session_state["product_type"] = "תיק לקוח"
                st.rerun()
...
def render_mix_builder(df_active, all_funds_list):
    ...
    with st.expander("⚙️ הגדרות מתקדמות", expanded=False):
        ...
        # Portfolio baseline status banner
        holdings_loaded = st.session_state.get("portfolio_holdings")
        baseline_loaded = st.session_state.get("portfolio_baseline")

        if holdings_loaded and baseline_loaded:
            n = len(holdings_loaded)
            total_h = sum(h.get("amount", 0) for h in holdings_loaded)
            bl = baseline_loaded

            # Check for frozen constraints set in client portfolio tab
            frozen_details = st.session_state.get("pf_frozen_details", [])
            frozen_line = ""
            if frozen_details:
                frozen_names = " · ".join(
                    f"{fd['fund'][:14]} ({fd['weight_pct']:.0f}%)"
                    for fd in frozen_details
                )
                frozen_line = (
                    f"<div style='font-size:11px;color:#92400E;margin-top:4px'>"
                    f"🔒 נעולות: {frozen_names}</div>"
                )

            _bl_s  = bl.get("stocks",0)
            _bl_f  = bl.get("foreign",0)
            _bl_fx = bl.get("fx",0)
            _bl_il = bl.get("illiquid",0)
            st.markdown(
                f"<div style='background:#EFF6FF;border:1px solid #3A7AFE;border-radius:8px;"
                f"padding:10px 14px;direction:rtl;margin-bottom:8px'>"
                f"<div style='font-weight:800;color:#1F3A5F;font-size:13px'>"
                f"✅ תיק נוכחי טעון כ-Baseline ({n} קרנות · ₪{total_h:,.0f})</div>"
                f"<div style='font-size:11px;color:#3A7AFE;margin-top:3px'>"
                "מניות " + f"{_bl_s:.1f}" + "% · " +
                'חו"ל ' + f"{_bl_f:.1f}" + "% · " +
                'מט"ח ' + f"{_bl_fx:.1f}" + "% · לא-סחיר " + f"{_bl_il:.1f}" + "%"
                f"<span style='margin-right:8px'>— חלופות יוצגו עם השוואה</span></div>"
                f"{frozen_line}"
                f"</div>",
                unsafe_allow_html=True,
            )
            ...
            with bcol1:
                if st.button("🗑️ נקה baseline", key="clear_baseline_btn",
                             help="מחק את הפורטפוליו הנוכחי כבסיס להשוואה"):
                    for k in ["portfolio_holdings","portfolio_baseline",
                              "portfolio_total","portfolio_managers",
                              "pf_frozen_details","pf_frozen_set","pf_multi_frozen"]:
                        st.session_state[k] = None if k not in ("portfolio_total",) else 0.0
                    st.session_state["locked_fund"] = ""
                    st.rerun()
            with bcol2:
                if frozen_details and st.button("🔓 נקה נעילות", key="clear_frozen_btn"):
                    st.session_state["pf_frozen_details"] = []
                    st.session_state["pf_frozen_set"]     = set()
                    st.session_state["locked_fund"]       = ""
                    st.rerun()
        elif holdings_loaded and not baseline_loaded:
            st.warning("⚠️ קרנות טעונות אך לא ניתן לחשב baseline — בדוק התאמה לנתוני המוצר הנבחר")
        else:
            st.markdown("""
<div style='background:#F8FAFC;border:1px dashed #D1D5DB;border-radius:8px;
padding:10px 14px;direction:rtl;font-size:12px;color:#6B7280'>
  💡 <b>טיפ:</b> עבור ל-📊 ניתוח תיק לקוח, העלה דוח מסלקה — 
  והאפליקציה תציג כאן השוואה אוטומטית לתיק הנוכחי שלך
</div>
""", unsafe_allow_html=True)
    ...
    # ── "Load from portfolio" button ─────────────────────────────────────
    baseline_now = st.session_state.get("portfolio_baseline")
    if baseline_now:
        _load_col, _hint_col = st.columns([2, 3])
        with _load_col:
            if st.button("📥 טען יעדים מהתיק הנוכחי",
                         key="load_targets_from_portfolio",
                         help="מזיז את הסליידרים לערכי התיק הנוכחי שהועלה — כנקודת פתיחה לעריכה"):
                tgts["stocks"]   = round(float(baseline_now.get("stocks",   tgts.get("stocks",   40.0))), 0)
                tgts["foreign"]  = round(float(baseline_now.get("foreign",  tgts.get("foreign",  30.0))), 0)
                tgts["fx"]       = round(float(baseline_now.get("fx",       tgts.get("fx",       25.0))), 0)
                tgts["illiquid"] = round(float(baseline_now.get("illiquid", tgts.get("illiquid", 20.0))), 0)
                st.session_state["targets"] = tgts
                st.rerun()
        with _hint_col:
            _bl_s  = baseline_now.get('stocks',0)
            _bl_f  = baseline_now.get('foreign',0)
            _bl_fx = baseline_now.get('fx',0)
            _bl_il = baseline_now.get('illiquid',0)
            st.caption(
                f"תיק נוכחי: מניות {_bl_s:.0f}% · "
                f'חו"ל {_bl_f:.0f}% · '
                f'מט"ח {_bl_fx:.0f}% · '
                f"לא-סחיר {_bl_il:.0f}%"
            )
    ...
    for key, label, default_on in params:
        ...
        with c1:
            ...
            # Show delta vs baseline if loaded
            bl_val = float(baseline_now.get(
                {"stocks":"stocks","foreign":"foreign","fx":"fx","illiquid":"illiquid"}.get(key, key), 0
            )) if baseline_now else None
        ...
        with c2:
            if bl_val is not None:
                ...
                st.markdown(
                    ...
                    f"<div style='font-size:14px;font-weight:800;color:#1F3A5F'>{val:.0f}%</div>"
                    f"<div style='font-size:11px;color:{delta_color};font-weight:700'>"
                    f"{delta_sign}{delta:.0f}pp</div>"
                    f"</div>",
                    unsafe_allow_html=True
                )
            else:
                st.markdown(
                    f"<div style='padding-top:28px;font-size:14px;font-weight:800;"
                    f"color:#1F3A5F;text-align:center'>{val:.0f}%</div>",
                    unsafe_allow_html=True
                )

    # ── Annuity / Capital target (shown only when product type is pension/gemel) ──
    _pt = st.session_state.get("product_type", "")
    _show_annuity = _pt in ("קרנות פנסיה", "קופות גמל", "גמל להשקעה")
    if _show_annuity or st.session_state.get("portfolio_holdings"):
        st.markdown("<div style='margin-top:8px;border-top:1px solid #E5E7EB;padding-top:8px'></div>",
                    unsafe_allow_html=True)
        _ac1, _ac2 = st.columns([3, 1])
        with _ac1:
            _ann_default = float(st.session_state.get("target_annuity_pct", 0.0))
            # Pre-fill from holdings if available
            if baseline_now and "annuity_pct" in baseline_now:
                _ann_default = float(baseline_now.get("annuity_pct", _ann_default))
            _ann_val = st.slider(
                "🏛️ קצבה (מתוך כלל התיק)",
                0.0, 100.0, _ann_default, step=5.0,
                key="sl_annuity",
                help="מה אחוז הנכסים שאמור להיות במוצרי קצבה (פנסיה, ביטוח מנהלים)?"
            )
            st.session_state["target_annuity_pct"] = _ann_val
        with _ac2:
            _cap_val = 100 - _ann_val
            st.markdown(
                f"<div style='padding-top:8px;text-align:center'>"
                f"<div style='font-size:11px;color:#6B7280'>קצבה</div>"
                f"<div style='font-size:14px;font-weight:800;color:#8B5CF6'>{_ann_val:.0f}%</div>"
                f"<div style='font-size:11px;color:#6B7280'>הון</div>"
                f"<div style='font-size:14px;font-weight:800;color:#06B6D4'>{_cap_val:.0f}%</div>"
                f"</div>",
                unsafe_allow_html=True
            )
    ...
    with btn_c1:
        run = st.button("🔍 חשב חלופה מיטבית",
                        type="primary", use_container_width=True, key="btn_calc")
    with btn_c2:
        run_strat = st.button("🔀 3 אסטרטגיות חלופיות",
                              type="secondary", use_container_width=True,
                              key="btn_strategies",
                              help="מחשב 3 חלופות: שמרנית · תפעולית · אגרסיבית · שארפ מקסימלי")

    return run, run_strat
...
<div class='card' style='text-align:center;padding:30px 18px'>
  ...
  <div style='font-size:13px;color:#6B7280'>הגדר יעדים ולחץ <b>חשב חלופות</b></div>
    ...
    # Annuity target gap (shown if user set a target)
    _ann_target = st.session_state.get("target_annuity_pct")
    _ann_chip_html = ""
    if _ann_target and _ann_target > 0:
        _ann_chip_html = (
            f"<div style='display:inline-flex;align-items:center;gap:4px;"
            f"background:#F5F3FF;border-radius:6px;padding:4px 10px;margin:2px;font-size:12px'>"
            f"<span style='color:#8B5CF6;font-weight:700'>🏛️ יעד קצבה: {_ann_target:.0f}%</span>"
            f"</div>"
        )
...
# ── AUTO-BASELINE: if holdings uploaded but baseline not yet computed ──
# This is the Step-0 connection: clearinghouse → optimizer baseline
if baseline is None and st.session_state.get("portfolio_holdings"):
    _holdings = st.session_state["portfolio_holdings"]
    _computed  = _compute_baseline_from_holdings(_holdings, df_active)
    if _computed:
        st.session_state["portfolio_baseline"] = _computed
        # Also pre-fill total_amount from holdings if not already set
        if not st.session_state.get("total_amount"):
            st.session_state["total_amount"] = sum(
                h.get("amount", 0) for h in _holdings
            )
        baseline = _computed
...
with col_left:
    run_clicked, run_strategies = render_mix_builder(df_active, all_funds)
...
# ── STRATEGY ALTERNATIVES RUN ──
if run_strategies:
    locked_pct = None
    if st.session_state["locked_fund"] and st.session_state["total_amount"] > 0 and st.session_state["locked_amount"] > 0:
        locked_pct = round(st.session_state["locked_amount"] / st.session_state["total_amount"] * 100, 1)
    current_managers = list(st.session_state.get("portfolio_managers") or [])

    with st.spinner("⚡ מחשב 3 אסטרטגיות (4 ריצות)..."):
        try:
            strat_results = _run_strategy_alternatives(
                df_active=df_active,
                n_funds=st.session_state["n_funds"],
                step=st.session_state["step"],
                include=st.session_state["include"],
...
if res and recs:
    ...
    # ── 3 Strategy Alternatives Display ─────────────────────────────────
    strat = st.session_state.get("strat_results", {})
    if strat:
        st.markdown("---")
        st.markdown("### 🔀 3 אסטרטגיות חלופיות")

        STRAT_META = [
            ("conservative", "🛡️ שמרנית",
             "מינימום שינויים · מנהלים נוכחיים · ציוני שירות גבוהים",
             "#EFF6FF", "#1D4ED8"),
            ("operational",  "⚙️ תפעולית",
             "שינויים פנימיים · מינימום ניודים · שירות גבוה",
             "#F0FDF4", "#15803D"),
            ("aggressive",   "🎯 אגרסיבית",
             "הכי קרוב ליעד · ללא התחשבות בשירות",
             "#FFF7ED", "#C2410C"),
            ("sharpe",       "📐 שארפ מקסימלי",
             "יחס תשואה/סיכון מיטבי · ללא קשר ליעד מדויק",
             "#F5F3FF", "#6D28D9"),
        ]

        s_cols = st.columns(4)
        for (key, title, desc, bg, fg), col in zip(STRAT_META, s_cols):
            r = strat.get(key)
            if r is None:
                with col:
                    st.markdown(
                        f"<div style='background:{bg};border:1px solid {fg}30;"
                        f"border-radius:10px;padding:14px;text-align:center;height:240px'>"
                        f"<div style='color:{fg};font-weight:800'>{title}</div>"
                        f"<div style='font-size:11px;color:#6B7280;margin:6px 0'>{desc}</div>"
                        f"<div style='color:#9CA3AF;font-size:12px;margin-top:40px'>אין תוצאות</div>"
                        f"</div>",
                        unsafe_allow_html=True,
                    )
                continue

            _s  = float(r.get("מניות (%)", 0) or 0)
            _f  = float(r.get('חו"ל (%)', 0) or 0)
            _fx = float(r.get('מט"ח (%)', 0) or 0)
            _il = float(r.get("לא־סחיר (%)", 0) or 0)
            _sh = float(r.get("שארפ משוקלל", 0) or 0)
            _sv = float(r.get("שירות משוקלל", 0) or 0)
            _sc = float(r.get("score", 0) or 0)
            _mgrs = str(r.get("מנהלים", ""))[:50]
            _wts  = _weights_short(r.get("weights"))
            ...
            def _d(val, bl_key):
                ...
                d = val - float(bv)
                c = "#10B981" if d > 0 else "#EF4444" if d < 0 else "#6B7280"
                s = "+" if d > 0 else ""
                return f"<span style='color:{c};font-size:10px'> {s}{d:.1f}pp</span>"

            with col:
                st.markdown(
                    f"<div style='background:{bg};border:1.5px solid {fg}40;"
                    f"border-radius:10px;padding:14px;min-height:280px'>"
                    f"<div style='color:{fg};font-weight:900;font-size:14px'>{title}</div>"
                    f"<div style='font-size:10px;color:#6B7280;margin:3px 0 10px'>{desc}</div>"
                    f"<div style='font-size:12px;line-height:1.8'>"
                    f"מניות: <b>{_s:.1f}%</b>{_d(_s,'stocks')}<br>"
                    f'חו"ל: <b>{_f:.1f}%</b>{_d(_f,'foreign')}<br>'
                    f'מט"ח: <b>{_fx:.1f}%</b>{_d(_fx,'fx')}<br>'
                    f"לא-סחיר: <b>{_il:.1f}%</b>{_d(_il,'illiquid')}<br>"
                    f"שארפ: <b>{_sh:.2f}</b><br>"
                    f"שירות: <b>{_sv:.0f}</b>"
                    f"</div>"
                    f"<div style='margin-top:8px;font-size:10px;color:#6B7280'>"
                    f"<b>מנהלים:</b> {_mgrs}<br>"
                    f"<b>משקלים:</b> {_wts}"
                    f"</div>"
                    f"</div>",
                    unsafe_allow_html=True,
        ...
        # Clear button
        if st.button("🗑️ נקה תוצאות אסטרטגיות", key="clear_strat"):
            st.session_state["strat_results"] = {}
            st.rerun()
...
# ── PORTFOLIO ANALYSIS (isolated module) ──
try:
    from portfolio_analysis.ui import render_portfolio_analysis
    render_portfolio_analysis(df_long, _product_type_now)
except Exception as _pf_err:
    st.expander("💼 ניתוח פורטפוליו נוכחי").error(f"שגיאת טעינה: {_pf_err}")

# ── INSTITUTIONAL STRATEGY ANALYSIS (isolated module) ──
try:
    ...
    _st.expander("📐 ניתוח אסטרטגיות מוסדיים").error(f"שגיאת טעינה: {_isa_err}")
